...
ggplot(dados, aes(y = setor_economico_bovespa)) + 
  geom_bar()
```

```{r}
# Sua vez: Faça o gráfico de barras para a variável ano
ggplot(dados, aes(y = ano)) + 
  geom_bar()
```
...
# gráfico
ggplot(dados, aes(market_to_book))+
  geom_histogram()
```
...
ggplot(dados, aes(x = ativo_total, fill = setor_economico_bovespa)) + 
  geom_boxplot() 
```
...
ggplot(dados, aes(x = endividamento, fill = trimestre)) + 
  geom_boxplot()
```
...
```{r}
ggplot(dados, aes(x = endividamento, y = roa)) + 
  geom_point()
```
...
```{r}
ggplot(dados, aes(x = endividamento, y = roa, colour = setor_economico_bovespa)) + 
  geom_point()
```
...
```{r}
ggplot(dados, aes(x = ativo_total, y = margem_operacional, colour = setor_economico_bovespa)) + 
  geom_point()
...
ggplot(dados, aes(x = endividamento, y = margem_operacional, colour = setor_economico_bovespa)) + 
  geom_point() + 
  geom_smooth(method = lm)
```
